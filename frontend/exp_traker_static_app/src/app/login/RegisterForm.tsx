...
// Types for form payload and API response error shape
...
type FieldError = {
  subjet: "email" | "username" | "password" | "firstName" | "lastName" | "birthDate";
  error: boolean;
  message: string;
};

type RegisterResponse = { error: boolean; errorData: FieldError[],message:String,status:String,userData:any };
...
export default function RegisterForm({ setIsLogin }: Props) {
  ...
  const handleRegister = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!payload.birthDate) nextErrors.birthDate = "Birth date is required";
    ...
    try {
      const res = (await RegisterUser(payload)) as RegisterResponse;
    console.log("registear",res);
      ...
      if ("errorData" in res && Array.isArray(res.errorData)) {
        ...
        for (const err of res.errorData) {
          ...
          const key =
            err.subjet === "username" ? "userName" : err.subjet; // normalize
            console.log(key);
